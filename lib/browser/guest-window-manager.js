...
ipcMainUtils.handle('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_METHOD', function (event, guestId, method, ...args) {
  ...
  if (!canAccessWindow(event.sender, guestContents)) {
    ...
    throw new Error(`Access denied to guestId: ${guestId}`)
  }
...
ipcMainUtils.handle('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_POSTMESSAGE', function (event, guestId, message, targetOrigin, sourceOrigin) {
...
ipcMainUtils.handle('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', function (event, guestId, method, ...args) {
  const guestContents = webContents.fromId(guestId)
  if (!guestContents) {
    throw new Error(`Invalid guestId: ${guestId}`)
  }
